...
local vouxsConfig = Window.ConfigManager:CreateConfig("vouxs")
...
local function Reg(id, element)
    vouxsConfig:Register(id, element)
...
local BaseFolder = "WindUI/" .. (Window.Folder or "vouxs") .. "/config/"
...
do
    ...
    _G.Vouxs_AutoAcceptTradeEnabled = false 
    ...
    if PromptController and PromptController.FirePrompt and Promise then
        ...
        PromptController.FirePrompt = function(self, promptText, ...)
            ...
            if _G.Vouxs_AutoAcceptTradeEnabled and type(promptText) == "string" and promptText:find("Accept") and promptText:find("from:") then
        ...
        warn("[Vouxs] Gagal memuat PromptController/Promise untuk Auto Accept Trade.")
    end
end
        ...
        if data then
            ...
            if targetPlayer.Character then
                for _, v in ipairs(targetPlayer.Character:GetChildren()) do
                    if v.Name == "VouxsESP" and v:IsA("BillboardGui") then pcall(function() v:Destroy() end) end
                end
            end
        end
    end
...
do
    ...
    task.spawn(function()
        ...
        if S1 and FishingController then
            ...
            FishingController.RequestChargeFishingRod = function(...)
                if _G.Vouxs_BlatantActive then return end 
                return Old_Charge(...)
            end
            FishingController.SendFishingRequestToServer = function(...)
                if _G.Vouxs_BlatantActive then return false, "Blocked by Vouxs" end
                return Old_Cast(...)
            end
        end
    end)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if _G.Vouxs_BlatantActive and not checkcaller() then
    ...
    local function SuppressGameVisuals(active)
        ...
        if active then
            task.spawn(function()
                ...
                while _G.Vouxs_BlatantActive do
...
local function GetItemsToFavorite()
    ...
    local accept = trade:Toggle({
        ...
        Value = false,
        Callback = function(state)
            _G.Vouxs_AutoAcceptTradeEnabled = state
...
do
    ...
    local function CreateKaitunUI()
        local old = game.CoreGui:FindFirstChild("VouxsKaitunStats")
        if old then old:Destroy() end
        local sg = Instance.new("ScreenGui")
        sg.Name = "VouxsKaitunStats"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                if not _G.BlackScreenGUI then
                    ...
                    _G.BlackScreenGUI.Name = "Vouxs_BlackBackground"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                for _, gui in ipairs(PlayerGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui.Name ~= "WindUI" and gui.Name ~= "CustomFloatingIcon_Vouxs" then
...
do
    ...
    local GLOBAL_WEBHOOK_URL = "https://discord.com/api/webhooks/1448964904118648937/d5Xeie2oIpdXbT9u6u6Wzxs40erqrYB8AWLgeqG46S8pwn88OyWqzVOua6XUSJ87B_C8"
    local GLOBAL_WEBHOOK_USERNAME = "Vouxs | Community"
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("<:TEXTURENOBG:1438662703722790992> Vouxs | Webhook\n\n<a:ChipiChapa:1438661193857503304> New Fish Caught! (%s)", fishName)
                ...
                local embed = {
                    ...
                    footer = {
                        text = string.format("Vouxs Webhook • Total Caught: %s • %s", caughtDisplay, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> Vouxs | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", fishName)
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("Vouxs Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[Vouxs Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    local SelectedConfigName = "Vouxs" -- Default
    local BaseFolder = "WindUI/" .. (Window.Folder or "Vouxs") .. "/config/"
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            ...
            -- 1. Save ke config utama dulu ("Vouxs.json")
            VouxsConfig:Save()
            task.wait(0.1)

            -- 2. Jika nama beda, salin isi "Vouxs.json" ke "NamaBaru.json"
            if SelectedConfigName ~= "Vouxs" then
                local success, err = pcall(function()
                    local mainContent = readfile(BaseFolder .. "Vouxs.json")
    ...
    ConfigSection:Button({
        ...
        Callback = function()
            if SelectedConfigName == "" or SelectedConfigName == "Vouxs" then 
            ...
            if isfile(path) then
                ...
                RefreshConfigList(ConfigDropdown)
                ConfigNameInput:Set("Vouxs")
                SelectedConfigName = "Vouxs"
            else
    ...
    --ConfigSection:Paragraph({
       ...
       -- Desc = "Script otomatis menyimpan ke 'Vouxs.json' setiap 30 detik.",
      --  Icon = "info"
 --   })
end
...
do
    ...
    about:Section({
        Title = "Join Discord Server Vouxs",
        TextSize = 20,
    })

    about:Paragraph({
        Title = "Vouxs Community",
        ...
        Buttons = {
            {
                ...
                Icon = "link",
                Callback = function()
                    setclipboard("https://discord.gg/Y6NvNycW")
                    WindUI:Notify({
                        Title = "Link Disalin!",
                        Content = "Link Discord Vouxs berhasil disalin.",
                        Duration = 3,
                        Icon = "copy",
                    })
                end,
...
local function CreateFloatingIcon()
    local existingGui = PlayerGui:FindFirstChild("CustomFloatingIcon_Vouxs")
    if existingGui then existingGui:Destroy() end

    local FloatingIconGui = Instance.new("ScreenGui")
    FloatingIconGui.Name = "CustomFloatingIcon_Vouxs"
...
WindUI:Notify({ Title = "Vouxs Was Loaded", Content = "Press [F] to open/close the menu", Duration = 5, Icon = "info" })
-- [[ AUTO LOAD & SAVE LOOP ]]
task.spawn(function()
    task.wait(5) -- Tunggu UI load sempurna
    
    -- Ganti Load biasa dengan Smart Load
    -- Default load "Vouxs", atau nama config terakhir user kalau kamu simpan
    SmartLoadConfig("Vouxs") 
    
    -- Auto Save Loop (Setiap 30 detik) -- Save tetap pakai cara biasa gapapa
    while true do
         task.wait(30)
         pcall(function() VouxsConfig:Save() end)
    end
end)
InitializeIcon()
